...
Function InitAllTasks()
	...
	LoadTask(cuminsidemouth, "op_sperm", "Cum inside mouth", 0)
	LoadTask(CumInsideVag, "op_impregnate", "Cum inside vagina", 0)
	LoadTask(CumInsideAnus, "op_sperm", "Cum inside ass", 0)
	LoadTask(ClientOrgasm, "op_heart", "Client orgasm", 3)
	LoadTask(PlayerOrgasm, "op_heart", "Prostitute orgasm", 2)

	LoadTask(OralSex, "op_oral", "Blowjob ", 0)
	LoadTask(VaginalSex, "op_vagina", "Vaginal intercourse", 1)
	LoadTask(AnalSex, "op_anal", "Anal sex", 1)
	LoadTask(HandjobSex, "op_handjob", "Handjob ", 2)
	LoadTask(EatPussySex, "op_cunnilingus", "Cunnilingus ", 1)
	LoadTask(VagPlaySex, "op_vagina", "Vaginal play ", 1)

	LoadTask(CowgirlSex, "op_cowgirl", "Cowgirl ", 1)
	LoadTask(KissingSex, "op_kissing", "Make out", 2)
	LoadTask(sixninesex, "op_69", "69 ", 3)

	LoadTask(AnythingGoes, "op_heart", "Anything goes", 2)
EndFunction
